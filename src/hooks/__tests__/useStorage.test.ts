...
describe('useStorage - listGames', () => {
  ...
  it('sets error and rethrows when listGames fails', async () => {
    ...
    await act(async () => {
      await expect(result.current.listGames()).rejects.toThrow('disk read error');
    });
...
describe('useStorage - getGameMetadata', () => {
  it('returns metadata for the given game ID', async () => {
    const meta = { game_id: 'g1', game_start_time: '2024-01-01', game_end_time: null, champion_name: 'Ahri', game_mode: 'CLASSIC' };
  ...
  it('sets error and rethrows when getGameMetadata fails', async () => {
    ...
    await act(async () => {
      await expect(result.current.getGameMetadata('g1')).rejects.toThrow('not found');
    });
...
describe('useStorage - saveGameMetadata', () => {
  it('calls storageApi.saveGameMetadata with correct arguments', async () => {
    ...
    const meta = { game_id: 'g1', game_start_time: '2024-01-01', game_end_time: null, champion_name: null, game_mode: null };
    const { result } = renderHook(() => useStorage());
...
describe('useStorage - deleteGame', () => {
  ...
  it('sets error string when deleteGame fails with a non-Error value', async () => {
    ...
    await act(async () => {
      await expect(result.current.deleteGame('game-xyz')).rejects.toBe('string-error');
    });
...
describe('useStorage - getStorageStats', () => {
  ...
  it('sets error when getStorageStats fails', async () => {
    ...
    await act(async () => {
      await expect(result.current.getStorageStats()).rejects.toThrow('storage unavailable');
    });
