...
describe('getErrorKey', () => {
  ...
  describe('string input', () => {
    it('maps known exact message to correct i18n key', () => {
      expect(getErrorKey('Invalid login credentials')).toBe('errors.invalidCredentials');
    });

    it('maps "User already registered" to emailAlreadyInUse', () => {
      expect(getErrorKey('User already registered')).toBe('errors.emailAlreadyInUse');
    });

    it('maps "Email rate limit exceeded" to tooManyRequests', () => {
      expect(getErrorKey('Email rate limit exceeded')).toBe('errors.tooManyRequests');
    });
  ...
  describe('Error instance input', () => {
    it('maps error code invalid_credentials to invalidCredentials', () => {
      const err = Object.assign(new Error('bad'), { code: 'invalid_credentials' });
    ...
    it('falls back to message matching when code is unknown', () => {
      const err = Object.assign(new Error('Invalid login credentials'), { code: 'unknown_code' });
      expect(getErrorKey(err)).toBe('errors.invalidCredentials');
    });
  ...
  describe('plain object input', () => {
    it('maps object with known code to correct key', () => {
      expect(getErrorKey({ code: 'invalid_email', message: 'bad email' })).toBe('errors.invalidEmail');
    });

    it('falls back to message matching when code is absent', () => {
      expect(getErrorKey({ message: 'User already registered' })).toBe('errors.emailAlreadyInUse');
    });
...
describe('getErrorKeyFromCode', () => {
  it('returns the mapped key for a known code', () => {
    expect(getErrorKeyFromCode('invalid_credentials')).toBe('errors.invalidCredentials');
  });

  it('returns the mapped key for rate_limit_exceeded', () => {
    expect(getErrorKeyFromCode('rate_limit_exceeded')).toBe('errors.tooManyRequests');
  });
...
describe('isAuthError', () => {
  it('returns true for an Error with a known auth error code', () => {
    const err = Object.assign(new Error('bad creds'), { code: 'invalid_credentials' });
  ...
  it('returns false for an Error with an unknown code', () => {
    const err = Object.assign(new Error('unknown'), { code: 'totally_unknown' });
