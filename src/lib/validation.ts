...
export function validateYoutubeUpload(input: YoutubeUploadInput): ValidationResult {
  ...
  if (input.tags && input.tags.length > 500) {
    errors.push({ field: 'tags', message: 'Tags must be 500 characters or less' });
  }

  const validPrivacyStatuses = ['public', 'unlisted', 'private'];
  if (!validPrivacyStatuses.includes(input.privacyStatus)) {
    errors.push({ field: 'privacyStatus', message: 'Privacy status must be public, unlisted, or private' });
  }
...
export function validateSettings(input: SettingsInput): ValidationResult {
  ...
  if (input.summoner_name !== undefined) {
    if (input.summoner_name.trim().length === 0) {
      errors.push({ field: 'summoner_name', message: 'Summoner name cannot be empty' });
    } else if (input.summoner_name.length > 32) {
      errors.push({ field: 'summoner_name', message: 'Summoner name must be 32 characters or less' });
    }
  }
