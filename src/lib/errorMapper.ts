...
const ERROR_MESSAGE_MAP: Record<string, string> = {
  ...
  'For security purposes, you can only request this once every 60 seconds': 'errors.tooManyRequests',
};
...
export function getErrorKeyFromCode(code: string): string {
  return AUTH_ERROR_MAP[code] || 'errors.generic';
}
...
export function isAuthError(error: unknown): boolean {
  if (error instanceof Error) {
    const supabaseError = error as Error & SupabaseError;
    return supabaseError.code !== undefined &&
