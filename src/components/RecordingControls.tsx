...
export function RecordingControls() {
  ...
  const warnings = readiness?.blockers.filter(b => b.severity === 'warning') || [];
  ...
  const getRecordingErrorMessage = (error: unknown): { title: string; description: string } => {
    const msg = getErrorMessage(error).toLowerCase();
    if (msg.includes('game') || msg.includes('lcu') || msg.includes('not detected') || msg.includes('not found') && msg.includes('process')) {
      return {
        title: t('errors.gameNotDetected'),
        description: t('errors.gameNotDetectedHint'),
      };
    }
    if (msg.includes('ffmpeg') || msg.includes('binary') || msg.includes('executable')) {
      return {
        title: t('errors.ffmpegNotFound'),
        description: t('errors.ffmpegNotFoundHint'),
      };
    }
    if (msg.includes('audio') || msg.includes('device') || msg.includes('microphone')) {
      return {
  ...
  return (
    <div data-testid="recording-controls" className="space-y-4">
      ...
      <div className="gaming-panel p-6">
        <div className="mb-4">
          <h3 className="text-lg font-semibold">{t('recordingControls.autoCapture.title')}</h3>
        ...
        {(isBlocked || hasWarnings) && (
          <Alert variant={isBlocked ? "destructive" : "default"} className="mb-4 bg-black/40 border-white/10">
            <AlertCircle className={`h-4 w-4 ${isBlocked ? 'text-red-500' : 'text-yellow-500'}`} />
            <AlertTitle className={isBlocked ? 'text-red-500' : 'text-yellow-500'}>
              {isBlocked ? t('recordingControls.readiness.blocked') : t('recordingControls.readiness.warning')}
            </AlertTitle>
      ...
      <div className="gaming-panel p-6">
        <div className="mb-4">
          <h3 className="text-lg font-semibold">{t('recordingControls.manualReplay.title')}</h3>
          <p className="text-sm text-muted-foreground">
