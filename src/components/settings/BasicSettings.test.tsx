...
import { BasicSettings, detectQualityLevel, detectSoundMode } from './BasicSettings';
import {
...
describe('BasicSettings', () => {
  ...
  describe('what to capture', () => {
    ...
    it('writes the canonical toggle combination when a preset is picked', async () => {
      ...
      expect(readState().event_filter).toEqual(HIGHLIGHT_PRESET_FILTERS.best_only);
    });

    it('falls back to "직접 설정" when an advanced toggle deviates', async () => {
      await renderUI(<Harness />);

      expect(screen.queryByTestId('highlights-custom-hint')).not.toBeInTheDocument();
  ...
  describe('quality', () => {
    it('maps a level onto resolution, frame rate and bitrate without touching the codec', async () => {
      await renderUI(<Harness />);

      click(
        screen.getByRole('radio', { name: 'settings.basic.quality.options.high.label' }),
      );
    ...
    it('shows custom when the advanced video panel deviates', async () => {
      ...
      expect(
        screen.getByRole('radio', { name: 'settings.basic.quality.options.medium.label' }),
      ).not.toBeChecked();
  ...
  describe('sound', () => {
    it('maps the three choices onto the two audio switches', async () => {
      ...
      click(
        screen.getByRole('radio', { name: 'settings.basic.sound.options.gameMic.label' }),
      );
      expect(readState().audio.record_microphone).toBe(true);
      expect(readState().audio.record_system_audio).toBe(true);

      click(
        screen.getByRole('radio', { name: 'settings.basic.sound.options.mute.label' }),
      );
    ...
    it('reports a mic-only combination as custom instead of lying', async () => {
      expect(
        detectSoundMode({ record_system_audio: false, record_microphone: true }),
      ).toBe('custom');
    });
  });
  ...
  it('toggles auto start with League', async () => {
    await renderUI(<Harness />);

    click(screen.getByLabelText('settings.basic.autoStart.label'));
    expect(readState().auto_start_with_league).toBe(false);
  });
  ...
  it('disables the controls while a save is in flight', async () => {
    ...
    expect(screen.getByLabelText('settings.basic.autoStart.label')).toBeDisabled();
  });

  it('treats a settings payload without the hidden filter fields as custom', async () => {
    // 미러가 아는 필드가 빠져 있으면 "균형"이라고 단정하지 않는다.
    const partial = { ...EVENT_FILTER_DEFAULTS } as Partial<typeof EVENT_FILTER_DEFAULTS>;
