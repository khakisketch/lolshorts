...
describe('AudioSettings', () => {
  beforeEach(() => {
    ...
    mockInvoke.mockImplementation((command: string) => {
      if (command === 'list_microphone_devices') return Promise.resolve(['Mic 1', 'Mic 2']);
      if (command === 'list_system_audio_devices') return Promise.resolve(['Speakers', 'Headset']);
  ...
  it('fetches device lists via list_microphone_devices and list_system_audio_devices', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('errors.audioNoDevicesAvailable')).not.toBeInTheDocument();
      expect(screen.queryByText('errors.audioDeviceNotFoundHint')).not.toBeInTheDocument();
    });
  });
  ...
  it('shows a warning when no audio devices are found at all', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('errors.audioNoDevicesAvailable')).toBeInTheDocument();
    });
  });
});
