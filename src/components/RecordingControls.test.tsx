...
jest.mock('@/api/settings', () => ({
  settingsApi: {
    getRecordingSettings: jest.fn().mockResolvedValue(createMockRecordingSettings()),
...
describe('RecordingControls', () => {
  ...
  test('start auto capture button is disabled when blocked', async () => {
    mockUseRecordingStore.mockReturnValue({
      ...
      readiness: {
        ready: false,
        blockers: [{ id: 'ffmpeg', message: 'FFmpeg not found', severity: 'critical' }],
        component_statuses: {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('recordingControls.readiness.blocked')).toBeInTheDocument();
  ...
  test('start auto capture button is enabled when only warnings exist', async () => {
    mockUseRecordingStore.mockReturnValue({
      ...
      readiness: {
        ready: false,
        blockers: [{ id: 'disk', message: 'Low disk space', severity: 'warning' }],
        component_statuses: {
    ...
    await waitFor(() => {
      const startButton = screen.getByText('recordingControls.autoCapture.start');
      expect(startButton).not.toBeDisabled();
      expect(screen.getByText('recordingControls.readiness.warning')).toBeInTheDocument();
