...
export function CanvasControlsPanel({
  ...
  const setBackgroundGradient = useCallback((color1: string, color2: string) => {
    onUpdateTemplate({ background: { type: 'Gradient', value: `${color1}:${color2}` } });
  }, [onUpdateTemplate]);
    ...
    <div className="w-80 border-l bg-card overflow-y-auto">
      <Tabs defaultValue="background" className="h-full">
        ...
        <TabsContent value="background" className="p-4 space-y-4">
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="Color">{t('editor.canvas.bgSolid')}</SelectItem>
                <SelectItem value="Gradient">{t('editor.canvas.bgGradient')}</SelectItem>
                <SelectItem value="Image">{t('editor.canvas.bgImage')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <TabsContent value="elements" className="p-4 space-y-4">
          ...
          {/* Element list */}
          <div className="space-y-2">
            <Label>{t('editor.canvas.elementsCount', { count: template.elements.length })}</Label>
