...
describe('Editor', () => {
  ...
  describe('Error States', () => {
    it('should display error when clips fail to load', async () => {
      // Mock error state
      jest.spyOn(require('@/hooks/useEditor'), 'useEditor').mockImplementation(() => ({
        loadGameClips: jest.fn(),
        isLoading: false,
        error: 'Failed to load clips',
      }));

      jest.spyOn(require('@/stores/editorStore'), 'useEditorStore').mockImplementation(() => ({
      ...
      await waitFor(() => {
        expect(screen.getByText('editor.errorLoadingClips')).toBeInTheDocument();
      });
    });
  });
});
