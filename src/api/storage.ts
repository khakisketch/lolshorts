...
export const storageApi = {
  ...
  getStorageStats: () => cmd<StorageStats>("get_storage_stats"),

  listClips: (gameId: string) =>
    cmd<ClipMetadata[]>("list_clips", { game_id: gameId }),

  listClipVaultPage: (input: {
    sort: ClipVaultSort;
    cursor: string | null;
    game_limit: number;
    query?: string;
  ...
  ensureClipThumbnail: (gameId: string, clipFilePath: string) =>
    cmd<string>("ensure_clip_thumbnail", {
      game_id: gameId,
  ...
  deleteGame: (gameId: string) => cmd<void>("delete_game", { game_id: gameId }),

  // Auto Edit Results
  getAutoEditResults: () =>
    cmd<AutoEditResultMetadata[]>("get_auto_edit_results"),

  getAutoEditResultGroups: () =>
    cmd<AutoEditResultGroup[]>("get_auto_edit_result_groups"),

  getAutoEditResult: (resultId: string) =>
    cmd<AutoEditResultMetadata>("get_auto_edit_result", {
      result_id: resultId,
    }),

  deleteAutoEditResult: (resultId: string, deleteFile: boolean) =>
    cmd<void>("delete_auto_edit_result", {
      result_id: resultId,
      delete_file: deleteFile,
    }),

  deleteAutoEditResultGroup: (seriesId: string, deleteFiles: boolean) =>
    cmd<void>("delete_auto_edit_result_group", {
      series_id: seriesId,
      delete_files: deleteFiles,
    }),

  updateAutoEditYoutubeStatus: (
    resultId: string,
    status: YouTubeUploadStatus,
  ) =>
    cmd<void>("update_auto_edit_youtube_status", {
      result_id: resultId,
      status,
    }),
};
