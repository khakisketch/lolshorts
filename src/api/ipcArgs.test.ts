 ...
 */
import { snakeKeyToCamel, isSupportedArgKey, toIpcArgs, type IpcArgWarning } from './ipcArgs';
...
describe('isSupportedArgKey', () => {
  it('accepts lowercase snake_case and lowerCamelCase', () => {
    for (const key of ['game_id', '_force_refresh', 'h264_level', 'gameId', 'title']) {
...
describe('toIpcArgs', () => {
  ...
  it('does NOT convert keys inside arrays', () => {
    expect(
      toIpcArgs({ clips: [{ file_path: 'a.mp4', trim_start: 1 }], output_path: 'out.mp4' })
    ).toEqual({
  ...
  it('reports collisions instead of silently dropping a value', () => {
    const warnings: IpcArgWarning[] = [];
    const out = toIpcArgs({ force_refresh: 1, forceRefresh: 2 }, (w) => warnings.push(w));
  ...
  it('stays silent for well-formed args', () => {
    const warnings: IpcArgWarning[] = [];
    toIpcArgs({ game_id: 'g', clip_file_path: 'c.mp4' }, (w) => warnings.push(w));
